...
CMOs can also designate tranches as interest-only (IO) or principal-only (PO).  IO tranches only receive interest payments, and POs only principal.  Remember standard fixed-rate mortgage payments are part interest and part principal.
...
See this presentation on securitization.
